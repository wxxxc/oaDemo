...
{
    public partial class ChooseUser_WorkDept : System.Web.UI.Page
    {
        ...
        public void DataBindToDownList()
        {
            ...
            SqlDataReader datar = SqlHelper.ExecuteReader("SELECT ui_id,ui_desc FROM yh WHERE ui_lx='企业' AND ui_id NOT IN (" + strlist2 + ") AND ui_ssbm IN (" + strlist + ")");
            while (datar.Read())
            {
                ListItem theItem = new ListItem();
                theItem.Text = datar["ui_desc"].ToString();
