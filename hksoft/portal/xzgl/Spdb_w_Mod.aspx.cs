...
{
    public partial class Spdb_w_Mod : System.Web.UI.Page
    {
        mydb hkdb = new mydb();
        ...
        protected void Set_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(Request.QueryString["id"].ToString()))
            {
                SqlHelper.ExecuteSql("UPDATE HK_SPDB SET YNXS='否'");
                SqlHelper.ExecuteSql("UPDATE HK_SPDB SET YNXS='是' WHERE SPID = '" + hkdb.GetStr(Request.QueryString["id"].ToString()) + "'");

                //提示 
                mydb.Alert_Refresh_Close("视频设置成功！","Spdb_w.aspx");
            }
        }
        ...
        protected void Del_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(Request.QueryString["id"].ToString()))
            {
                ...
                mydb.Alert_Refresh_Close("删除成功！","Spdb_w.aspx");
            }
        }
    }
}
