...
{
    public partial class Kqsmqr2_View : System.Web.UI.Page
    {
        mydb hkdb = new mydb();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    SqlDataReader dr = SqlHelper.ExecuteReader("SELECT DISTINCT * from OA_KQ_SM02 A LEFT OUTER JOIN OA_KQ_SM01 B ON(A.SSSID=B.SID) WHERE B.SID='" + hkdb.GetStr(Request.QueryString["id"].ToString()) + "' AND A.SPRID='" + this.Session["userid"] + "' AND A.SPZT='确认完成'");
                    if (dr.Read())
                    {
