...
namespace hksoft
{
    public partial class myMain : PageBase
    {
        #region Page_Init

        public string msgicon = "";
        public static string RemTimeStr;
        protected void Page_Init(object sender, EventArgs e)
        {
            ...
            //菜单生成
            initMenu();
            int msgnum = (Int32)SqlHelper.ExecuteScalar("SELECT COUNT(*)AS NUMS FROM hk_Mess_JS WHERE jsr='" + this.Session["userid"] + "' AND zt='0'");
            if (msgnum == 0)
            {
                msgicon = "";
            }
        ...
        {
            msg = "";
            int msgcount = (Int32)SqlHelper.ExecuteScalar("SELECT COUNT(*)AS NUMS FROM hk_Mess_JS WHERE jsr='" + this.Session["userid"] + "' AND zt='0'");
            if (msgcount > 0)
            {
                ...
                {
                    string content = "";
                    if (dr["nr"].ToString().Length > 70)
                    {
                        content = dr["nr"].ToString().Substring(0, 70) + "...";
                    }
                    else
                    {
                        content = dr["nr"].ToString();
                    }
                    msg = msg + "<a href=\"#\" onclick=\"showmsg('" + dr["id"].ToString() + "','" + dr["ui_desc"].ToString() + "','" + dr["nr"].ToString() + "','"
                        + dr["fssj"].ToString() + "');\"><div class=\"btn btn-danger btn-circle\"><i class=\"ti ti-email\" style=\"font-size:18px;\"></i></div>"
                        + "<div class=\"mail-contnet\" style=\"margin-left: 10px;width:80%;\"><h5>" + dr["ui_desc"].ToString() + "</h5><span class=\"mail-desc\" style=\"white-space: normal;\">" + content
                        + "</span> <span class=\"time\">" + dr["fssj"].ToString() + "</span></div></a>";
                }
                dr.Close();
            }
            else
            {
                msg = "<span style=\"margin-left:30px;margin-top:20px;display:block;color:red;\">暂无消息</span>";
            }
        }
        ...
        private void initMenu()
        {
            ...
            foreach (DataRow row in ds.Tables[0].Rows)
            {
                ...
                {
                    menu = menu + "<li><a class=\"has-arrow waves-effect waves-dark\" href=\"#\" aria-expanded=\"false\"><i class=\""
                        + row["modu_icon"].ToString() + "\"></i><span class=\"hide-menu\">" + row["modu_mc"].ToString() + " </span></a>";
                    string submenu = ResolveSubMenu(row);
        ...
        private string ResolveSubMenu(DataRow dataRow)
        {
            string submenu = "";
            DataRow[] rows = dataRow.GetChildRows("TreeRelation");
            if (rows.Length > 0)
            {
                submenu = submenu + "<ul aria-expanded=\"false\" class=\"collapse\">";
                foreach (DataRow row in rows)
                {
                    submenu = submenu + "<li><a data-addtab=\"tab_" + row["modu_id"].ToString()
                        + "\" data-url=\"" + row["modu_wjlj"].ToString() + "\" href=\"#" + row["modu_wjlj"].ToString() + "\"><i class=\""
                        + row["modu_icon"].ToString() + "\" style=\"margin-right:5px;\"></i>" + row["modu_mc"].ToString() + "</a></li>";
                    //更下级菜单
                    //string ss=ResolveSubMenu(row);
                }
                submenu = submenu + "</ul>";
            }
            return submenu;
        ...
        public static string strLogo, strDate, userImg, userName, userDep, userPos, score, menu;

        protected void btn_getmsgs_Click(object sender, EventArgs e)
        {
            //获取未读消息
            initMsg();
        }
        ...
        //已读消息
        protected void btn_readmsg_Click(object sender, EventArgs e)
        {
            SqlHelper.ExecuteSql("UPDATE hk_Mess_JS SET zt='1' WHERE jsr='" + this.Session["userid"] + "' AND id='" + hkdb.GetStr(txt_msgid.Text) + "'");
        }
