...
{
    public partial class Qjdqr2_View : System.Web.UI.Page
    {
        mydb hkdb = new mydb();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    SqlDataReader dr = SqlHelper.ExecuteReader("SELECT DISTINCT * from OA_KQ_QJD02 A LEFT OUTER JOIN OA_KQ_QJD01 B ON(A.SSQID=B.QID) WHERE B.QID='" + hkdb.GetStr(Request.QueryString["id"].ToString()) + "' AND A.SPRID='" + this.Session["userid"] + "' AND A.SPZT='确认完成'");
                    if (dr.Read())
                    {
