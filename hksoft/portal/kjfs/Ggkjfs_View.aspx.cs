...
{
    public partial class Ggkjfs_View : System.Web.UI.Page
    {
        mydb hkdb = new mydb();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    lbltitle.Text = "修改";
                    SqlDataReader datar = SqlHelper.ExecuteReader("SELECT * FROM hk_Quick WHERE QLX='Y' AND QID='" + hkdb.GetStr(Request.QueryString["id"].ToString()) + "'");
                    if (datar.Read())
                    {
        ...
        public void DropDownListBind()
        {
            ...
            bd.Bind_DropDownList(tb, sql, "TBLJ", "TBLJ", "null.gif", "缺省图标");
        }
        ...
        protected void Save_Click(object sender, EventArgs e)
        {
            ...
            if (lbltitle.Text == "新增")
            {
                if (qlb.SelectedValue == "2")
                {
                    ...
                    if (fjtype1 == "exe" | fjtype1 == "EXE" | fjtype1 == "Exe")
                    {
                        ...
                        int newID = (Int32)SqlHelper.ExecuteScalar("INSERT INTO HK_QUICK(QMC,QLJ,QTB,QJQM,CJSJ,QLX,QLB) VALUES('" + hkdb.GetStr(mc.Text) + "','" + FileName + "','" + tb.SelectedValue + "','" + Dns.GetHostName() + "',GETDATE(),'Y','" + qlb.SelectedValue + "')" + "SELECT CAST(scope_identity() AS int)");
                    ...
                    {
                        mydb.Alert("必须选择扩展名为.exe的可执行文件！");
                        return;
                    }
                }
                else
                {
                    SqlHelper.ExecuteSql("INSERT INTO HK_QUICK(QMC,QLJ,QTB,QJQM,CJSJ,QLX,QLB) VALUES('" + hkdb.GetStr(mc.Text) + "','" + hkdb.GetStr(fj.Text) + "','" + tb.SelectedValue + "','" + Dns.GetHostName() + "',GETDATE(),'Y','" + qlb.SelectedValue + "')");
                }
            }
            else
            {
            ...
            {
                Clear_Click();
                mydb.Alert_Refresh("保存成功,请新增下一条记录！", "Ggkjfs.aspx");
            }
            else
            {
                mydb.Alert_Refresh_Close("保存成功！","Ggkjfs.aspx");
            }
        }
