...
String.prototype.toDate = function (style) {
    ...
    //获取选择月份最大日期，梅花雨在获取today变量时获取的是上一次选择的日
    //如果上次选择31，当切换到2月份或者非最大日期不是31的月份，用31构造日部分月份会进位导致月份切换了
    ////////////2017-10-31 fixed by showbo http://www.w3dev.cn/

    var dateFirst = new Date(parseInt(y), parseInt(M, 10) - 1, 1);
    dateFirst.setMonth(dateFirst.getMonth() + 1); dateFirst.setDate(dateFirst.getDate() - 1);
    var maxDay = dateFirst.getDate();
    d = Math.min(d, maxDay);
    ////////////////////////
...
Calendar.prototype.draw = function () {
    ...
    obj.onclick = function () {
        var dateFirst = new Date(calendar.year, calendar.month, 1);
        dateFirst.setMonth(dateFirst.getMonth() + 1); dateFirst.setDate(dateFirst.getDate() - 1);
        var maxDay = dateFirst.getDate();
        calendar.day = Math.min(calendar.day, maxDay);
...
{
 var str = '<div id="calendarPanel" style="position: absolute;display: none;z-index:9999; background-color: #FFFFFF;border: 1px solid #CCCCCC;width:60%;font-size:12px;"></div>';
 if(document.all)
 {
