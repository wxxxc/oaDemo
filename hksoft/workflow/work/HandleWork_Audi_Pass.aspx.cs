    ...
    public partial class HandleWork_Audi_Pass : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (dr.Read())
                {
                    lblstrfnName.Text = dr["NextNodeName"].ToString();
                    lblstrwSerialNo.Text = dr["wSerialNo"].ToString();
                ...
                SqlDataReader datar3 = SqlHelper.ExecuteReader("SELECT A.*,B.ui_desc FROM hk_WorkAppComm A LEFT OUTER JOIN yh B ON(A.FK_uid=B.ui_id) WHERE ac_wfID IN (SELECT WID FROM hk_WORK WHERE wNo='" + hkdb.GetStr(Request.QueryString["Number"]) + "') AND ac_Status='有效' ORDER BY acID");
                while (datar3.Read())
                {
        ...
        public void BindLb(string str)
        {
            ...
            if (dr.Read())
            {
                if (dr["fnPosition"].ToString() == "结束")
                {
                    ...
                    lblmsg1.Text = "当前为结束步骤";
                    lblmsg2.Text = "当前为结束步骤";
                    lblmsg3.Text = "当前为结束步骤";
                    lblmsg4.Text = "当前为结束步骤";
                ...
                else
                {
                    ...
                    {
                        lblmsg1.Text = "<a href=\"javascript:void(0)\" onclick=\"openUser()\" class=\"gvlink\">[选择审批人员]</a>";
        ...
        protected void rblFormName_SelectedIndexChanged(object sender, EventArgs e)
        {
            txt_wUserNames.Text = "";
            txt_wUserIDs.Text = "";
        }
