...
{
    public partial class Bmjh_Gzx_Add : System.Web.UI.Page
    {
        mydb hkdb = new mydb();
        ...
        protected void Save_Click(object sender, EventArgs e)
        {
            ...
            if (s == "SaveAddBtn")
            {
                ...
                {
                    mydb.Alert_Refresh("保存成功,请新增下一条记录！", "Bmjhzd_Mod.aspx?id=" + Request.QueryString["id"] + "");
                }
                else if (Request.QueryString["type"].ToString() == "a")//新增页面
                {
                    mydb.Alert_Refresh("保存成功,请新增下一条记录！", "Bmjhzd_Add.aspx?id=" + Request.QueryString["id"] + "");
                }
            }
            else
            {
                if (Request.QueryString["type"].ToString() == "m")//修改页面
                {
                    mydb.Alert_Refresh_Close("保存成功！","Bmjhzd_Mod.aspx?id=" + Request.QueryString["id"] + "");
                }
                else//新增页面
                {
                    mydb.Alert_Refresh_Close("保存成功！","Bmjhzd_Add.aspx?id=" + Request.QueryString["id"] + "");
                }
            }
        }
