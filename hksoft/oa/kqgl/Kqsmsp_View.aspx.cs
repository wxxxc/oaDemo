...
{
    public partial class Kqsmsp_View : System.Web.UI.Page
    {
        mydb hkdb = new mydb();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    SqlDataReader dr = SqlHelper.ExecuteReader("SELECT A.* FROM OA_KQ_SM01 A WHERE SID='" + hkdb.GetStr(Request.QueryString["id"].ToString()) + "' AND CHARINDEX('" + this.Session["userid"] + "',jsrIDs)>0 AND SMZT='送审中'");
                    if (dr.Read())
                    {
        ...
        protected void Pass_Click(object sender, EventArgs e)
        {
            ...
            using (TransactionScope scope = new TransactionScope())
            {
                ...
                sqlComm = new SqlCommand("INSERT INTO OA_KQ_SM02(SSSID,JSSJ,SPRID,SPRMC,SPSJ,SPYJ,BS,SPZT) values('" + hkdb.GetStr(Request.QueryString["id"].ToString()) + "','" + hkdb.GetStr(sssj.Text) + "','" + this.Session["userid"] + "','" + this.Session["username"] + "',getdate(),'" + hkdb.GetStr(spyj.Text) + "','1','" + strSpzt + "')", sqlConn);
                sqlComm.CommandType = System.Data.CommandType.Text;
                sqlComm.ExecuteNonQuery();

                sqlComm = new SqlCommand("UPDATE OA_KQ_SM01 SET SMZT='" + strSqzt + "',jsrIDs='" + srjsrIDs + "',jsrNames='" + strjsrNames + "',SSSJ=GETDATE() WHERE SID='" + hkdb.GetStr(Request.QueryString["id"].ToString()) + "'", sqlConn);
                sqlComm.CommandType = System.Data.CommandType.Text;
                sqlComm.ExecuteNonQuery();
            ...
            //PrnLbl.Visible = true;
            //Image3.Visible = true;
            mydb.Alert_Refresh_Close("审批通过成功！","Kqsmsp.aspx");
        }
        ...
        protected void Back_Click(object sender, EventArgs e)
        {
            using (TransactionScope scope = new TransactionScope())
            {
                ...
                int n = Convert.ToInt32(SqlHelper.ExecuteScalar("SELECT COUNT(*)AS NUMS FROM OA_KQ_SM02 WHERE SSSID='" + hkdb.GetStr(Request.QueryString["id"].ToString()) + "' AND BS='1'"));
                //退给录入人
                if (n < 1)
                {
            ...
            mydb.Alert_Refresh_Close("审批退回完成！","Kqsmsp.aspx");
        }
        ...
        public void SendMsg()
        {
            ...
            if (xxtz.Checked == true | dxtz.Checked == true)
            {
                if (jsr.SelectedValue == "jdqr")
                {
                    string message = this.Session["username"].ToString() + "审批通过了一份考勤说明,请确认!";
                    hkdb.SendMsg(xxtz.Checked, dxtz.Checked, this.Session["userid"].ToString(), kqry.SelectedValue, message);
                }
                else
                {
                    string message = this.Session["username"].ToString() + "审批通过了一份考勤说明,请继续审批!";
        ...
        protected void Kqspsm()
        {
            ...
            if (dr0.Read())
            {
                ...
                {
                    SqlDataReader dr = SqlHelper.ExecuteReader("SELECT SMID FROM OA_KQ_SPSM WHERE SMLX='考勤说明单' AND SYRY='考勤审批分管领导'");
                    if (dr.Read())
                    {
                        lcsm.Visible = true;
                ...
                else
                {
                    ...
                    {
                        SqlDataReader dr = SqlHelper.ExecuteReader("SELECT SMID FROM OA_KQ_SPSM WHERE SMLX='考勤说明单' AND SYRY='部门考勤审批人员'");
                        if (dr.Read())
                        {
                            lcsm.Visible = true;
                            lcsm.Attributes.Add("onclick", "Javascript:popPrn('Kqspsm_Show.aspx?id=" + dr["SMID"].ToString() + "');");
