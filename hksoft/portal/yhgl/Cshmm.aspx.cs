...
{
    public partial class Cshmm : System.Web.UI.Page
    {
        mydb hkdb = new mydb();
        ...
        protected void Set_Click(object sender, EventArgs e)
        {
            ...
            SqlDataReader dr = SqlHelper.ExecuteReader("SELECT UI_PWD FROM YH WHERE UI_ID='" + hkdb.GetStr(yhid.Text) + "' AND UI_LX='企业'");
            if (dr.Read())
            {
                dr.Close();
                ...
                SqlHelper.ExecuteSql("UPDATE YH SET UI_PWD='" + EnPswdStr + "' WHERE UI_ID='" + hkdb.GetStr(yhid.Text) + "' AND UI_LX='企业'");
        ...
        {
            SqlDataReader dr = SqlHelper.ExecuteReader("SELECT UI_DESC FROM YH WHERE UI_ID='" + hkdb.GetStr(yhid.Text) + "' AND UI_LX='企业'");
            if (dr.Read())
            {
