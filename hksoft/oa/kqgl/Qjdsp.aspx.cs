...
{
    public partial class Qjdsp : System.Web.UI.Page
    {
        mydb hkdb = new mydb();
        ...
        {
            string Str1 = " WHERE CHARINDEX('" + this.Session["userid"] + "',jsrIDs)>0 AND QJZT='送审中'";

            if (!string.IsNullOrEmpty(qjlx.SelectedValue))
            {
                Str1 = Str1 + " AND QJLX='" + qjlx.SelectedValue + "'";
            }
            ...
            AspNetPager1.RecordCount = (int)SqlHelper.ExecuteScalar("SELECT COUNT(*)AS NUMS FROM OA_KQ_QJD01 A" + Str1);
            DataSet ds = SqlHelper.ExecuteDs("SELECT A.* FROM OA_KQ_QJD01 A " + Str1 + " ORDER BY SSSJ DESC", AspNetPager1.PageSize * (AspNetPager1.CurrentPageIndex - 1), AspNetPager1.PageSize);
        ...
        protected void Pass_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(cal))
            {
                ...
                using (TransactionScope scope = new TransactionScope())
                {
                    ...
                    SqlDataReader dr = SqlHelper.ExecuteReader("SELECT A.QID,A.SSSJ FROM OA_KQ_QJD01 A WHERE QID IN('" + cal + "')");
                    while (dr.Read())
                    {
                        sqlComm = new SqlCommand("INSERT INTO OA_KQ_QJD02(SSQID,JSSJ,SPRID,SPRMC,SPSJ,SPYJ,BS,SPZT) values('" + dr["qid"].ToString() + "','" + dr["sssj"].ToString() + "','" + this.Session["userid"] + "','" + this.Session["username"] + "',getdate(),'" + hkdb.GetStr(spyj.Text) + "','1','" + strSpzt + "')", sqlConn);
                        sqlComm.CommandType = System.Data.CommandType.Text;
                        sqlComm.ExecuteNonQuery();

                        sqlComm = new SqlCommand("UPDATE OA_KQ_QJD01 SET QJZT='" + strSqzt + "',jsrIDs='" + srjsrIDs + "',jsrNames='" + strjsrNames + "',SSSJ=GETDATE() WHERE QID='" + dr["qid"].ToString() + "'", sqlConn);
                        sqlComm.CommandType = System.Data.CommandType.Text;
                        sqlComm.ExecuteNonQuery();
                    }
                    dr.Close();
                ...
                //PrnLbl.Visible = true;
                //Image3.Visible = true;
                mydb.Alert_Goto("批量审批成功！", "Qjdsp.aspx");
            }
            else
            {
        ...
        public void SendMsg()
        {
            ...
            if (xxtz.Checked == true | dxtz.Checked == true)
            {
                if (jsr.SelectedValue == "jdqr")
                {
                    string message = this.Session["username"].ToString() + "批量审批通过了一份请假单,请确认!";
                    hkdb.SendMsg(xxtz.Checked, dxtz.Checked, this.Session["userid"].ToString(), kqry.SelectedValue, message);
                }
                else
                {
                    string message = this.Session["username"].ToString() + "批量审批通过了一份请假单,请继续审批!";
