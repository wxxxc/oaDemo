...
{
    public partial class Bmjhzd_Add : System.Web.UI.Page
    {
        mydb hkdb = new mydb();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    SqlDataReader dr = SqlHelper.ExecuteReader("select a.*,b.bm_mc from oa_gzjh01 a,BM b where a.jhbm=b.bm_id and jhid='" + hkdb.GetStr(Request.QueryString["id"].ToString()) + "'");
                    if (dr.Read())
                    {
                        jhnf.Text = dr["jhnf"].ToString();
                        jhlx.SelectedValue = dr["jhlx"].ToString();
                        ztmb.Text = dr["ztmb"].ToString();
                        bm.SelectedValue = dr["jhbm"].ToString();
                    ...
                    AddBtn.Attributes.Add("onclick", "Javascript:popMod('bmjh_gzx_add.aspx?type=a&id=" + Request.QueryString["id"] + "');");
                }
        ...
        {
            IDlabel.Text = Convert.ToString(SqlHelper.ExecuteScalar("insert into OA_GZJH01(jhnf,jhyf,ztmb,zt,jhz,lrsj,jhlx,jhsx,jhbm) values('" + hkdb.GetStr(jhnf.Text) + "','" + yf.SelectedValue + "','" + hkdb.GetStr(ztmb.Text) + "','录入','" + this.Session["userid"] + "',GETDATE(),'" + jhlx.SelectedValue + "','1','" + bm.SelectedValue + "')" + "select cast(scope_identity() as int)"));
            mydb.Alert_Refresh("保存成功！", "bmjhzd.aspx");
            ...
            AddBtn.Attributes.Add("onclick", "Javascript:popMod('bmjh_gzx_add.aspx?type=a&id=" + IDlabel.Text + "');");
        }
        ...
        protected void Del_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(cal))
            {
                ...
                mydb.Alert_Refresh("删除成功！", "bmjhzd.aspx");
                GridViewBind();
            }
            else
            {
