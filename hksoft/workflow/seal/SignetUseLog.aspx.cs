...
{
    public partial class SignetUseLog : System.Web.UI.Page
    {
        mydb hkdb = new mydb();
        ...
        public void GridViewBind()
        {
            ...
            AspNetPager1.RecordCount = (int)SqlHelper.ExecuteScalar("SELECT COUNT(*) FROM hk_SealLog WHERE FK_uid='" + this.Session["userid"] + "'" + Str1);
            DataSet ds = SqlHelper.ExecuteDs("SELECT A.*,B.ui_desc FROM hk_SealLog A LEFT OUTER JOIN yh B ON(A.FK_uid=B.ui_id) WHERE FK_uid='" + this.Session["userid"] + "'" + Str1 + " ORDER BY slID DESC", AspNetPager1.PageSize * (AspNetPager1.CurrentPageIndex - 1), AspNetPager1.PageSize);
