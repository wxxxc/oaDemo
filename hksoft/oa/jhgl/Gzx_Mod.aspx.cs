...
{
    public partial class Gzx_Mod : System.Web.UI.Page
    {
        mydb hkdb = new mydb();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (dr.Read())
                {
                    ...
                    sjwcsj.Text = string.Format("{0:d}", dr["sjwcsj"]);
        ...
        protected void Save_Click(object sender, EventArgs e)
        {
            ...
            {
                mydb.Alert_Refresh_Close("保存成功！", "Wdjh_mod.aspx?id=" + hkdb.GetStr(Request.QueryString["jhid"]) + "");
            }
            else//新增页面过来
            {
                mydb.Alert_Refresh_Close("保存成功！", "Wdjh_add.aspx?id=" + hkdb.GetStr(Request.QueryString["jhid"]) + "");
            }
        }
        ...
        protected void SavePj_Click(object sender, EventArgs e)
        {
            ...
            if (Request.QueryString["type"].ToString() == "m")//修改页面
            {
                mydb.Alert_Refresh_Close("保存成功！", "Wdjh_mod.aspx?id=" + Request.QueryString["jhid"] + "");
            }
            else//新增页面过来
            {
                mydb.Alert_Refresh_Close("保存成功！", "Wdjh_add.aspx?id=" + Request.QueryString["jhid"] + "");
            }
        }
    }
}
