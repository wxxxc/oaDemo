...
{
    public partial class Kqsmsp2_View : System.Web.UI.Page
    {
        mydb hkdb = new mydb();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    SqlDataReader dr = SqlHelper.ExecuteReader("SELECT DISTINCT * from OA_KQ_SM02 A LEFT OUTER JOIN OA_KQ_SM01 B ON(A.SSSID=B.SID) WHERE B.SID='" + hkdb.GetStr(Request.QueryString["id"].ToString()) + "' AND A.SPRID='" + this.Session["userid"] + "' AND A.SPZT IN('审批通过','审批退回','等待确认')");
                    if (dr.Read())
                    {
