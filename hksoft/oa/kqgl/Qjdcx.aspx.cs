...
{
    public partial class Qjdcx : System.Web.UI.Page
    {
        mydb hkdb = new mydb();
        ...
        protected void Exp_Click(object sender, EventArgs e)
        {
            ...
            DataSet ds = SqlHelper.ExecuteDs("SELECT A.sqrmc as '请假人',qjlx as '类型',qjkssj as '开始日期',qjkssd as '开始时段',qjjssj as '截止日期',qjjssd as '截止时段',qjts as '请假天数',qjzt as '状态',qjly as '请假理由',gztzqk as '工作调整情况',sqbmmc as '所属部门',sqsj as '录入时间' FROM OA_KQ_QJD01 A " + sql + " ORDER BY SQSJ DESC");

            string fileName = "Qjdcx.xls";
            ExcelHelper.ExportDataSetToExcel(ds, fileName, "请假单查询");
        }
    }
}
