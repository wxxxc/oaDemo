...
{
    public partial class Bmjhzd_Mod : System.Web.UI.Page
    {
        mydb hkdb = new mydb();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (dr.Read())
                {
                    ny.Text = dr["jhnf"].ToString() + "年" + dr["jhyf"].ToString() + dr["bm_mc"].ToString() + " 工作计划";
                ...
                switch (zt.Text)
                {
                    case "录入":
                        ...
                        //增加按钮 
                        AddBtn.Attributes.Add("onclick", "Javascript:popView('bmjh_gzx_add.aspx?type=m&id=" + Request.QueryString["id"] + "');");
                        istr = "lr";
                        break;
                    case "审核中":
                        istr = "shz";
        ...
        {
            SqlHelper.ExecuteSql("update OA_GZJH01 set ztmb='" + hkdb.GetStr(ztmb.Text) + "' where jhid='" + hkdb.GetStr(Request.QueryString["id"]) + "'");
        ...
        protected void Up_Click(object sender, EventArgs e)
        {
            ...
            //if (xxtz.Checked == true | dxtz.Checked == true)
            //{
            //    string message = this.Session["username"].ToString() + "向您提交了一份部门工作计划的自我评价,请审批!";
            //    hkdb.SendMsg(xxtz.Checked, dxtz.Checked, this.Session["userid"].ToString(), spr.SelectedValue, message);
            //}
            mydb.Alert_Refresh_Close("自我评价已成功提交！", "bmjhzd.aspx");
        }
        ...
        protected void DelJh_Click(object sender, EventArgs e)
        {
            ...
            mydb.Alert_Refresh_Close("删除计划成功！", "bmjhzd.aspx");
        }
