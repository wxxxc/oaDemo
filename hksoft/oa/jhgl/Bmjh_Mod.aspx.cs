...
{
    public partial class Bmjh_Mod : System.Web.UI.Page
    {
        mydb hkdb = new mydb();
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                SqlDataReader dr = SqlHelper.ExecuteReader("select * from oa_gzjh01 a,BM b where a.jhbm=b.bm_id and jhid='" + hkdb.GetStr(Request.QueryString["id"].ToString()) + "'");
                if (dr.Read())
                {
        ...
        protected void Sp_Click(object sender, EventArgs e)
        {
            ...
            {
                string message = this.Session["username"].ToString() + "已审批了您的部门工作计划,请查看!";
                hkdb.SendMsg(xxtz.Checked, dxtz.Checked, this.Session["userid"].ToString(), ycxx.Text, message);
            }

            mydb.Alert_Refresh_Close("计划已审批完成！", "Bmjh.aspx");
        }
        ...
        protected void Pj_Click(object sender, EventArgs e)
        {
            ...
            if (xxtz.Checked == true | dxtz.Checked == true)
            {
                string message = this.Session["username"].ToString() + "为您的部门工作计划作了评价,请查看!";
                hkdb.SendMsg(xxtz.Checked, dxtz.Checked, this.Session["userid"].ToString(), ycxx.Text, message);
            }

            mydb.Alert_Refresh_Close("计划已评价完成！", "Bmjh.aspx");
        }
