...
{
    public partial class MySeal_Use : System.Web.UI.Page
    {
        mydb hkdb = new mydb();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                SqlDataReader dr = SqlHelper.ExecuteReader("SELECT * FROM hk_WorkFlowSeal WHERE FK_wfno='" + hkdb.GetStr(Request.QueryString["FlowNumber"]) + "' AND fsNo='" + hkdb.GetStr(Request.QueryString["yznum"]) + "' AND (CHARINDEX('," + this.Session["userid"].ToString() + ",',','+fsUserIDs+',') > 0 )");
                if (!dr.Read())
                {
                    mydb.Alert_Close("您无权操作此印章！");
                    return;
                }
                dr.Close();
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            if (dr.Read())
            {
                ...
                this.Response.Write("<script language=javascript>window.opener.setpic('" + Request.QueryString["picname"] + "','" + dpSeal.SelectedValue + "');window.close();</script>");
            }
            else
            {
