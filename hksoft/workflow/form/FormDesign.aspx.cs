...
namespace hkpro.workflow.form
{
    public partial class FormDesign : System.Web.UI.Page
    {
        ...
        public void GridViewBind()
        {
            ...
            AspNetPager1.RecordCount = (int)SqlHelper.ExecuteScalar("SELECT COUNT(*)AS NUMS FROM hk_Form A LEFT OUTER JOIN hk_FormClass B ON(A.FK_fcid=B.fcID)" + Str1);
            DataSet ds = SqlHelper.ExecuteDs("SELECT A.*,B.fcName FROM hk_Form A LEFT OUTER JOIN hk_FormClass B ON(A.FK_fcid=B.fcID)" + Str1, AspNetPager1.PageSize * (AspNetPager1.CurrentPageIndex - 1), AspNetPager1.PageSize);
        ...
        //{
        //    mydb.Goto("FormDesign_Mod.aspx?id=" + chkChoose() + "");
        //}
        ...
        protected void Del_Click(object sender, EventArgs e)
        {
            ...
            int i = 0;
            i = (int)SqlHelper.ExecuteScalar("SELECT COUNT(*) FROM hk_WorkFlow WHERE FK_fid IN (" + chkStr + ")");
            if (i > 0)
            {
                mydb.Alert("删除失败,选中的部分表单已经和流程绑定！");
                return;
            }
            else
            {
                SqlHelper.ExecuteSql("DELETE FROM hk_Form WHERE fID IN(" + chkStr + ")");
                mydb.Alert("删除成功！");
            }
            GridViewBind();
        }
