...
{
    public partial class zsdtlb : System.Web.UI.Page
    {
        mydb hkdb = new mydb();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                SqlDataReader datar = SqlHelper.ExecuteReader("select * from OA_KlClassSub where  id='" + hkdb.GetStr(Request.QueryString["LittleId"]) + "'  ");
                if (datar.Read())
                {
                    Namefile = datar["Name"].ToString();
                }
                datar.Close();
        ...
        {
            AspNetPager1.RecordCount = (int)SqlHelper.ExecuteScalar("select count(id) from  OA_KnowLedge where State='正常' and ((CHARINDEX('," + this.Session["userid"].ToString() + ",',','+QxYdUsername+',')>0) or QxYdUsername='全部用户')  and  LittleId='" + hkdb.GetStr(Request.QueryString["LittleId"]) + "'");
            DataSet ds = SqlHelper.ExecuteDs("select * from OA_KnowLedge where State='正常' and ((CHARINDEX('," + this.Session["userid"].ToString() + ",',','+QxYdUsername+',') > 0 ) or QxYdUsername='全部用户')   and  LittleId='" + hkdb.GetStr(Request.QueryString["LittleId"]) + "' order by LastTime desc", AspNetPager1.PageSize * (AspNetPager1.CurrentPageIndex - 1), AspNetPager1.PageSize);
