...
{
    public partial class AdminDoc_OfficeEdit : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                SqlDataReader dr = SqlHelper.ExecuteReader("SELECT adName FROM hk_AdminDoc WHERE adNewName='" + hkdb.GetStr(Request.QueryString["file"]) + "'");
                if (dr.Read())
                {
                    adname = dr["adName"].ToString();
                }
                dr.Close();
