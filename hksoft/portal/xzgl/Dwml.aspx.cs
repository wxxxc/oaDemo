...
{
    public partial class Dwml : System.Web.UI.Page
    {
        mydb hkdb = new mydb();
        ...
        public void OperQx()
        {
            hkdb.QxValid(AddBtn, "11302a", Session["roleqx"].ToString());
        ...
        public void GridViewBind()
        {
            ...
            AspNetPager1.RecordCount = (int)SqlHelper.ExecuteScalar("SELECT COUNT(*) FROM hk_DWML" + Str1);
            DataSet ds = SqlHelper.ExecuteDs("SELECT * FROM hk_DWML" + Str1, AspNetPager1.PageSize * (AspNetPager1.CurrentPageIndex - 1), AspNetPager1.PageSize);
        ...
        protected void Del_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(cal))
            {
                SqlHelper.ExecuteSql("DELETE FROM HK_DWML WHERE ID IN('" + cal + "')");
                mydb.Alert("删除成功！");
                GridViewBind();
            }
            else
